...
AtomNomrodView = require './atom-nimrod-view'
{CompositeDisposable,Directory,File} = require 'atom'

module.exports = AtomNomrodView =
	atomNomrodView: null
	modalPanel: null
	subscriptions: null
	...
	executeCommand: (command, callback) ->
		...
		suppress = atom.config.get('nimrod.suppressPanel')
	...
	activate: (state) ->
		@atomNomrodView = new AtomNimrodView(state.atomNimrodViewState)
		# @modalPanel = atom.workspace.addModalPanel(item: @atomNimrodView.getElement(), visible: false)
		...
		@subscriptions.add atom.workspace.observeTextEditors (editor)=>
			...
			@subscriptions.add editor.onDidSave (event)=>
				try
					@executeOn(event.path,'.nimrod.json')
				catch error
					console.log error
	...
	loadConfig: (editorPath, filename,callback)->
		...
		timeout 	= atom.config.get('nimrod.timeout')	# Load global configurations
		commands 	= atom.config.get('nimrod.commands')
		@config = {}
	...
	deactivate: ->
		...
		@atomNimrodView.destroy()

	serialize: ->
		atomNimrodViewState: @atomNimrodView.serialize()

	executeOn: (path,configFile)->
		@killPanel()
		suppressPanel = atom.config.get('nimrod.suppressPanel')	# Load global configurations
